...
from .config import PROMPTS, AVAILABLE_MODELS


def stream_response(prompt_name: str, input_text: str, user_response: Optional[str] = None,
                    model: str = "gemini-1.5-flash", api_key: Optional[str] = None) -> Generator[str, None, None]:
    ...
    if not prompt_text:
        yield f"Erreur: Mode '{prompt_name}' non reconnu."
        return

    if prompt_name == "repondre":
